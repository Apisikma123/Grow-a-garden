...
@section('title', 'Ganti Password — Grow a Garden')
@section('description', 'Perbarui password akun Anda.')

@section('dashboard-content')
    <div class="flex flex-col gap-[24px] pb-10">
        <div>
            <div class="flex items-center gap-3 mb-2">
                <a href="/settings" class="w-10 h-10 rounded-full bg-surface-container-high flex items-center justify-center text-on-surface-variant hover:bg-outline-variant/30 hover:text-on-surface transition-colors">
                    <span class="material-symbols-outlined">arrow_back</span>
                </a>
                <h1 class="text-[32px] md:text-[48px] font-bold text-on-surface tracking-tight leading-tight">Ganti Password</h1>
            </div>
            <p class="text-[16px] text-on-surface-variant leading-[24px] ml-[52px]">Pastikan akun Anda tetap aman dengan menggunakan password yang kuat.</p>
        </div>
            ...
            <div class="bg-surface rounded-[24px] p-8 ambient-shadow-lg border border-outline-variant/20">
                <form action="/settings/password" method="POST" class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="pt-4 flex items-center gap-3 md:col-span-2">
                        ...
                        <a href="/settings" class="flex-1 text-center text-primary font-bold py-3.5 hover:bg-primary/5 rounded-full transition-colors text-[14px]">
                            Batal
                        </a>
                    </div>
